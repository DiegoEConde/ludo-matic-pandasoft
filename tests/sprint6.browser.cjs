...
async function main(){
  ...
  const artifacts=path.join(os.tmpdir(),process.env.LUDO_ARTIFACTS || 'ludo-sprint6');fs.mkdirSync(artifacts,{recursive:true});
  try{for(const [width,height,reduced] of JSON.parse(process.env.LUDO_VIEWPORTS || '[[1366,768,false],[1024,600,false],[390,844,false],[320,640,true]]')){
    ...
    const roll=async(value,id)=>{
      ...
      assert.equal(await p.locator('#six-celebration').isVisible(),value===6);
      assert((await p.locator('#dice-message').textContent()).includes(' '+value+'.'));
      assert(await p.locator('#dice-modal-action').evaluate(e=>e===document.activeElement));
      await p.keyboard.press('Tab');
      await p.keyboard.press('Shift+Tab');
      assert(await p.locator('#dice-modal').evaluate(e=>e.contains(document.activeElement)));
      if(reduced) assert.equal(await p.locator('#six-celebration').evaluate(e=>getComputedStyle(e).animationName),'none');
    ...
    // No-move help stays available, with manual continuation and automatic fallback.
    await p.evaluate(()=>{Math.random=()=>0});
    await p.locator('#dice-control').click(); await p.clock.runFor(920);
    assert((await p.locator('#dice-message').textContent()).includes('Necesitas un 6'));
    await p.clock.runFor(4000); assert.equal(await phase(),'roll-result');
    await p.clock.runFor(4100); assert.equal(await phase(),'waiting-roll');
    assert((await p.locator('#game-status').textContent()).includes('Necesitas un 6'));
    ...
    for(let i=0;i<4;i++){
      ...
      assert.equal(await p.locator('#game-status').getAttribute('data-feedback'),'capture');
      assert(await card(color+'-1').evaluate(e=>e.classList.contains('just-captured')));
    ...
    for(let number=1;number<=4;number++){
      ...
      assert.equal(await p.locator('#game-status').getAttribute('data-feedback'),'finish');
      assert(await card(id).evaluate(e=>e.classList.contains('just-finished')));
    ...
    assert.deepEqual(errors,[]);console.log('PASS Sprint 6 '+width+'x'+height+' reduced='+reduced);await c.close();
